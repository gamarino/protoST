...
## Resolution (2026-05-24 — same day)

Race ROOT-CAUSED and FIXED in commit `7637e4f`. Diagnosis:
`PROTOST_SCHED_DIAG=1` revealed the exact lost-wakeup pattern in
under a minute (item 1 of the punch list above):

```
appendFutureWaiter future=... parked=0 (already settled)
schedule actor=driver marked wakeup (1->2)
drainOne FRESH-YIELD actor=driver
finishDrain RELEASE (stale wakeup) actor=driver   ← BUG
```

The 2026-05-23 stale-wakeup optimisation (commit `f21aab4`)
released the actor whenever `finishDrain` found `schedState==2`
with an empty mailbox. Correct for state=2 generated by a
redundant SEND ordering — WRONG when state=2 was generated by the
yield path itself (a settle that beat the actor's
`appendFutureWaiter` registration, then re-marked state 1→2 to
re-schedule the actor). The fix is one-liner: re-enqueue when
`suspended=true`, not just when `mailboxHasWork`. The
hoisted-block / captured-dict invariants (items 2 of the punch
list) turned out to be red herrings — the race was entirely in
the scheduler, not the snapshot path.

**Item 3 — multi_producer numbers**: ran end-to-end after the
fix. NOT the 10× the design speculated. The multi-producer
pattern yields a yield-resume cycle per fan-out element, which
is MORE expensive than main's blocking wait. Measured (best of
3, 5500U, MemoryMax=12G):

| workers | wall | msg/s | note |
|---|---|---|---|
| 1 | 3.58 s | 27 K | baseline |
| 2 | 2.37 s | 41 K | 1.51× |
| 4 | 1.89 s | 51 K | **best** — 1.88× |
| 6 | 2.25 s | 43 K | — |
| 8 | 1.95 s | 49 K | — |

mt100a single-producer baseline w=6 hits ~ 85 K msg/s. So
multi-producer is **comparable, not multiplicative**. The real
value of `doYielding:` is **correctness + composability** — a
driver-actor pattern that was previously unwriteable (deadlocked
on yield-in-`do:`) now runs end-to-end. The README's headline
performance projections stay at the 0.2.0 figure
(mt100a w=2 ~ 67-72 K msg/s class). When a workload genuinely
demonstrates a multi-producer multiplier we will revisit the
projection.

### Implementation status (FINAL)

- `tests/conformance/do_yielding.st` — basic correctness (passes).
- `tests/conformance/do_yielding_actor.st` — race fix regression
  (driver-actor + `doYielding:` + `wait` inside, multi-worker
  stable).
- `benchmarks/actors/multi_producer.st` — rewritten end-to-end,
  produces the correct answer (48 048 000), measures realistic
  throughput.
- `docs/tutorial/10-actors-and-futures.md` §10.8 — worked example
  for users.
- `docs/LANGUAGE.md` — `doYielding:` reference paragraph next to
  `do:`.
- `CHANGELOG.md` — 0.3.0 entry.
- 753/753 ctest stable. Version bumped 0.2.0 → 0.3.0.
