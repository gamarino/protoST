...
- **Test suite:** 807 `ctest` cases (330 conformance, 42 examples, 10 CLI,
  425 unit), counted with `ctest -N` on 2026-09-15; 807/807 pass with the
  D28 fix. Before it: 797/797 with the S6 fix, 793/793 with the D25, D26
  and D27 fixes, and 786 cases before those.
...
| Id | Bug | Severity |
|----|-----|----------|
| D29 | **The compiler crashes on block literals in some inlined `to:do:` bodies.** Repro (each a whole script): `1 to: 2 do: [ :i \| [ 1 ] value ]`, `1 to: 2 do: [ :i \| [ :e \| 2 ] ]`, `1 to: 2 do: [ :i \| [ 1 ] on: Error do: [ :e \| 2 ] ]`, and the nested loop `s := 0. 1 to: 2 do: [ :i \| #(1 2) do: [ :x \| s := s + x ] ]`. `protost` dies before the program runs and prints nothing: a segmentation fault (for the first repro in `Compiler::tryEmitInlinedControl`, called from `compileModule`), or a floating-point exception for the nested loop. The nested loop also crashes inside a method body. Seen to work: the same block expression when its value is assigned (`1 to: 2 do: [ :i \| x := [ :e \| e ] value: i ]`), block literals in `#(1 2) do: [ … ]` and `true ifTrue: [ … ]` bodies, a `to:do:` body without a block literal, and a method whose `to:do:` is itself inside a non-inlined block. Found on 2026-09-15 while writing the D28 tests, on builds before and after D28's fix; not investigated further. | High |
