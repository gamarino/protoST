...
- **Test suite:** 797 `ctest` cases (320 conformance, 42 examples, 10 CLI,
  425 unit), counted with `ctest -N` after reconfiguring on 2026-09-15;
  797/797 pass with the S6 fix. Before it: 793/793 with the D25, D26 and
  D27 fixes, and 786 cases before those.
...
| Id | Bug | Severity |
|----|-----|----------|
| D28 | **An error raised inside an imported module's top level reaches the importer's handler twice.** Repro: `_raising_mod.st` contains `Error signal: 'boom in module'.`; the importer runs `[ Import from: '_raising_mod' ] on: Error do: [ :e \| e messageText printNl ]`. The handler runs once with `boom in module`, then a second time with `exception unwind: no matching on:do: handler activation`, and that second value is the result of `on:do:`. Present before S6 (same output from a build of `e169fd9`) and on every thread. Root cause: `loadModuleFromFile` runs the module through `runTopLevel`, whose catch clauses turn `UnwindToHandler` (and `NonLocalReturn`, `RetrySignal`, `ResumeSignal`, `PassSignal`) into a `std::runtime_error`; those clauses are meant for the outermost script, but here the target frame is the importer's `on:do:`, outside the module. The provider then re-signals the `runtime_error` as a new `Error`. Fix direction: run a module's top level without the outermost-script conversions so control-flow signals reach the importer's frames. `conformance/11-modules/import-cycle-concurrent.st` records the first error its handler sees so that it holds before and after this fix. | Medium |
