...
- **Test suite:** 854 `ctest` cases (361 conformance, 42 examples, 14 CLI,
  437 unit), counted with `ctest -N` on 2026-09-24; 854/854 pass after the
  Track Y fix (S17), which added three unit cases over a cross-runtime UMD
  import. Before it: 851/851 after the
  S15 and S16 fixes, which added one garbage-collection unit case and two CLI
  fixtures (one proving that a collection reclaims, one proving that an actor
  payload only the mailbox references survives the collections that run while
  it is queued — the Track S test that could not be written until protoST
  could collect, and which found S16 on its first outing). Before it: 848/848 after Track S,
  which added two mailbox-reachability unit cases, nine conformance cases
  pinning the D32 decision and four white-box cases over the ProtoMap-backed
  hashed collections (one conformance case is an expected failure pinning D30).
...
- **Open bugs:** two (D30 and S3, both Medium). S15 closed on 2026-09-24; the
  same fix very probably closes S3 as well, but that could not be proved on
  this build — see S3's own entry.
...
| D21 | **`addBehavior:` affects future instances only.** `aClass addBehavior: aMixin` makes the class object and every instance created *after* the call respond to the mixin's methods; an instance created *before* the call does **not** gain them. | protoCore captures an object's parent chain into its base cell at construction and the object never re-reads it, so a later `addParent`/`setParents` on the class is invisible to instances that **already exist** (verified by direct probing against protoCore 2.0.0). `addBehavior:` rebuilds the class with the mixin baked into the base chain and rebinds the global; the rebuilt class is a new object, and the instances that already exist keep the old one in their chains. Method *attributes* installed directly on a class (via `>>`) **are** seen by pre-existing instances — only new *parents* are not. Lifting D21 would require protoCore to let a constructed object observe later parent mutations of its prototype, which it deliberately does not do: the captured chain is what makes lookup allocation-free and snapshot-stable. *(Under protoCore 1.x even instances created **after** the mutation could not see a new parent, because `newChild` copied the prototype's birth-state chain; protoCore 2.0.0 copies its current chain instead — CHANGELOG "Upgrading from 1.2.0", item 4 — so a plain `addParent` on a live class now also reaches future instances. That does not change D21, whose subject is pre-existing instances, but it does mean the rebuild is no longer the only route to "future instances" semantics; it is kept because the resolution order, the `super` lookup and the global rebind are specified against it.)* Documented in `LANGUAGE.md` §4.12. *(Borderline: a real constraint born of a protoCore design choice; classified intentional because protoST exposes a genuinely useful capability — runtime behaviour composition with no recompilation — within the constraint rather than working around it.)* |
...
|----|------|------------|--------|
| S17 | **`provider:st` could not serve a caller in another runtime's `ProtoSpace`**, so no cross-runtime import of a protoST module resolved. `ModuleProvider::tryLoad(path, ctx)` receives the **caller's** context, and `STModuleProvider` resolved its runtime from `ctx->space` through the S6 space registry — which is a space protoST does not own when the caller is a co-resident runtime. The lookup found nothing and the provider returned `PROTO_NONE`, so a module that was there was reported as absent. Measured by protoScala on 2026-09-24 against `3fd0438`: an `STRuntime` and a protoScala `Session` in one process, both providers reachable, and `import st.counter_lib` answering `provider 'st' has no module 'counter_lib'` while the identical call from a context in protoST's own space loaded it. | **The cause is in the provider, not in protoCore, and protoCore is unchanged.** A `ModuleProvider` is an object with its own state. A caller in a space protoST owns takes the unchanged S6 path. A caller in a space protoST does not own is a cross-runtime import: the runtime comes from `soleSTRuntime()` — the registry's single entry, read rather than cached, because the provider is registered once per process and outlives every `STRuntime`, so a pointer captured at construction would dangle for any host that builds a runtime per test. Two rules follow and both were found by test, not by reasoning: the load must run in protoST's **own** space (running a module's top level on a foreign context interns its literals in the foreign symbol table and leaves protoST's own later lookups missing — D26/T5-a's trap one level up), and the module **namespace** must be rebuilt with keys interned in the **caller's** space, because an attribute key is an interned symbol's address and protoCore interns per `ProtoSpace`. Only the mapping is rebuilt; every value is the protoST object itself, by address, which is what makes "no copy at the boundary" true rather than asserted. Because protoCore embeds a short string in the pointer word, a 5-byte member name matched across spaces **by accident** and a 7-byte one missed **silently**, so the tests use a class called `Counter` and compare the two symbol addresses. A cross-runtime load runs on protoST's root context, which only the constructing thread may allocate on (D26); an import from any other thread is refused with a message (`STRuntime::isOwnerThread`), and a process with two `STRuntime`s is refused as ambiguous. **Proof:** `tests/unit/test_cross_runtime_provider.cpp`, which needs no sibling runtime — a bare `proto::ProtoSpace` is a space protoST does not own — and prints the same cell's address and `getHash` read from both spaces; and protoScala's `umd/protost-interop`, which drives it from a real second runtime and forces a collection in each space. Checked against mutations: disabling `soleSTRuntime`, and returning protoST's own module object instead of the re-keyed namespace, each turn the relevant assertions red; cloning the values turns the no-copy assertions red. **What this does NOT deliver** is in INTEROP §4.2: values cross, a **call** does not (a protoST method is `__bc_ptr__` plus protoST's engine, not a `proto::ProtoMethod`); one importing thread; one protoST runtime; a namespace snapshot. | `e82682b` |
| S16 | **An actor mailbox batch lost its only GC root when a turn adopted a second batch**, so a collection mid-turn freed the messages the turn had not reached yet and the next `MailboxCursor::next()` read a freed cell. Segfault, about two runs in five under a heap ceiling; `ProtoList::getAt` from `STRuntime::drainOne` on a worker thread. | `MailboxCursor::adopt` re-pinned the batch with `pin_.reset(new TransientPin(...))` on a `std::unique_ptr`. `std::unique_ptr::reset` constructs the replacement *before* destroying what it replaces, so the second adopt in a turn — a `__pending__` tail followed by a queue batch, or two non-empty `takeAll`s — released a scratch slot above one still in use. That left `g_scratchCursor` pointing at the live pin's own slot, so the next `TransientPin` anywhere on that thread claimed it and overwrote the batch pointer. (`TransientPin`'s destructor asserts LIFO release, but the assert is compiled out of a release build.) The cursor now owns one `TransientPin` by value for its whole lifetime and re-points it with `TransientPin::reset`, which claims no new slot. Introduced by the mailbox migration (`6d4acb1`) and invisible until S15 was fixed, because a runtime that reclaims nothing cannot expose a lost root — which is precisely the gap Track S recorded. Found by `tests/cli/test_cli_actor_payload_gc.sh`, which is the Track S mailbox-reachability test that could not be written before S15; it launches the workload eight times, and the pre-fix binary segfaults. | `3dbd039` |
| S15 | **Nothing a protoST program allocated was ever reclaimed.** `ProtoSpace::triggerGC()` left `getGCCycleCount()` at 0 with and without actors, and any `PROTOCORE_HEAP_LIMIT_CELLS` above protoST's live set aborted a plain `1 to: 100000 do: [ :i | Array new: 8 ]` with "last cycle reclaimed 0 — out of memory". | **Cause, established by measurement, not by the filed hypothesis: protoST never submitted its young generation.** protoCore chains every cell a context allocates onto that context's young generation; the collector's Phase-2 root scan records the chain head as a root and the concurrent mark walks the whole chain, so an unsubmitted chain is live by definition. A context submits its chain in exactly two places — when it is destroyed, and from `ProtoContext::safepoint()` past `maxAllocatedCellsPerContext` (10,000 cells). protoST creates no `ProtoContext` of its own: a program runs on the runtime's root context and an actor turn on its worker's root context, both alive for the life of the process, and the interpreter called `safepoint()` nowhere at all. Measured on the pre-fix build: after four runs of a 20,000-iteration allocating loop the root context's chain held 2,748,398 cells, cycles *did* run (10, then 17, then 22 of them) and every one reclaimed exactly 0 while the heap grew monotonically. **The two secondary observations in the report are protoCore behaving as designed, not part of the bug:** `triggerGC()` is advisory (it raises the request only when the free-cell ratio is under 0.2 or a request is already pending), and `PROTOCORE_GC_PROFILE=1` prints nothing unless protoCore is built with `-DPROTOCORE_GC_INSTRUMENT=ON`, which no build here is. The filed hypothesis — an idle actor worker blocking the stop-the-world quorum, as in protoClojure — did **not** hold: protoST's worker already waits inside a `ProtoContext::UnmanagedScope`, which is why the symptom was identical with and without actors. **Fix:** `ExecutionEngine::gcSafepoint` calls `ProtoContext::safepoint()` at the loop back-edge (`Op::JUMP_BACK`) and at engine entry — the two points at which the engine holds no half-built value in a C++ local, every live object being in a frame region or a `TransientPin` scratch slot, both of them traced `automaticLocals`. That is exactly the precondition `safepoint()` documents. After it, the same workload holds the heap flat at 1,048,576 cells with a steady ~242,000-cell live set and reclaims ~730,000 cells per cycle. `PROTOST_NO_GC_SAFEPOINT=1` disables the hook; the worst-case cost, a 20,000,000-iteration integer loop that does nothing but add, is +2.7 % instructions and +4 % cycles (`perf stat -r 3`). **Proof:** `tests/unit/test_gc_cycle.cpp` ("S15: a forced collection completes and reclaims") and `tests/cli/test_cli_gc_reclaims.sh`, which runs the workload a second time with the hook disabled and fails if *that* run succeeds. With the hook off the unit case reports 5 cycles, 0 cells reclaimed and a heap grown from 1,048,576 to 3,866,624 cells. | `3dbd039` |
| D32 | Hashed collections did not agree on element equality. A `Set` stored a protoCore `ProtoSet`, whose membership is by protoCore's hash, so `1` and `1.0` were two elements although `1 = 1.0`, and all NaNs were one element although `Float nan = Float nan` is false. A `Dictionary` bucketed keys by protoCore's hash, so a key `1` was not found as `1.0`. A `Bag` compared with `=` and found both. `LANGUAGE.md` §9.6-9.8 did not say which equality decides membership. | **Decision [agent, pending review]:** the language's `=`, with identity as a fast path, decides membership in every hashed collection, and the hash is numeric-aware so that equal numbers of different kinds share a slot. This is the first of the three options D32 itself listed. Implemented by moving `Set` and `Dictionary` onto protoCore's `ProtoMap` and its hashed-collection helper, with protoST supplying the `KeySemantics`: `equals` is `a == b or: [a partialCompare: b) = 0]` — the same predicate `Bag` and `OrderedCollection` already used — and `hash` sends every number through its double value (`-0.0` normalised, strings still canonicalised through the symbol table, everything else keeping `getHash`, which for a plain object is identity). **Consequences:** a `Set` holding `1` now includes `1.0`; a `Dictionary` keyed by `1` answers for `1.0`; `Bag` is unchanged, because that is already what it did; and a `Set` holding one NaN no longer includes a *different* NaN, since identity is the only way a NaN is ever found (which is what D31 requires and the opposite of the old hash-only behaviour). **Why this option:** `=` is the equality the language already documents (§12.2 states value equality across the numeric tower), it is what `Bag` and every sequenceable collection already used, and it is what a Smalltalk programmer expects; the other two options — protoCore hash equality everywhere, or documenting the difference — would have made `Bag` the odd one out or frozen a disagreement the language has no reason to have. **Cost of reversing:** low but not free. The equality and hash live in two functions (`stKeyEquals`, `stKeyHash`) behind one `KeySemantics`, so the rule changes in one place; what would have to change with it is nine conformance cases in `tests/conformance/09-collections/` that pin the new behaviour, three paragraphs of `LANGUAGE.md` §9.6-9.8, and any user code that has come to rely on `1` and `1.0` being one key. The representation itself (`ProtoMap` plus the helper) is independent of the decision and would stay. | `91090fd` |
...
| S13 | The unit test "F6 v3 E3: cooperative chain survives aggressive GC on 4 workers" failed intermittently with `Future rejected: doesNotUnderstand: + (receiver: nil)` (4 runs in 300; 12 in 300 on the build before S4). More generally, an actor suspended in `wait` could resume before the awaited future settled, and `wait` answered `nil`: a message sent to the actor while it waited, or one that arrived during the turn in which it suspended, was enough. | Root cause, from a `PROTOST_SCHED_DIAG` trace: a SEND during the turn that suspended the actor marked its `__sched__` flag 2; the turn's drain loop consumed that message, and `finishDrain` re-queued the suspended actor (the rule for a future that settled before the waiter registered). The flag does not record why it was raised, and `drainOne`'s resume path resumed without checking the awaited future, pushing `nil` as the value of `wait`. Fix: when `__waiting_on__` is still pending, `drainOne` leaves the actor suspended (snapshot, `__waiting_on__` and the waiter registration untouched) and returns; the future's settle reschedules it, and the existing three-state flag covers a settle racing that turn. Verified by `conformance/10-actors/resume-waits-for-awaited-future.st` (5/5 failing before, also with 1 and 2 workers; passing after with 1, 2 and 8 workers) and 0 failures in 1000 runs of the E3 4-worker case. 833/833 `ctest` (default configuration). | `25a496f` |
| S14 | A fresh configure could link a stale protoCore: `CMakeLists.txt` searched `../protoCore/build` before `build_release`, and picked a June library left in `build/` over the current `build_release` one. | The search order is `build_release`, `build`, `build_check` (as in protoClojure), and the not-found message suggests `build_release`; the README documents the order and the `PROTOCORE_LIBRARY` / `PROTO_CORE_PREFIX` overrides. Verified with fresh configures before (library from `build/`) and after (from `build_release/`). 833/833 `ctest`. | `e31e545` |
