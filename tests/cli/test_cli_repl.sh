#!/usr/bin/env bash
# F7: interactive REPL (protost -i). Drives the REPL by piping stdin and
# asserting on captured stdout. Piped stdin is non-tty, so the REPL takes the
# std::getline fallback path (readline is reserved for the interactive tty).
set -euo pipefail
PROTOST="$1"

# Temporary files go to $TMPDIR, or to the build tree (Testing/Temporary next
# to the protost binary) when TMPDIR is unset — never to a hard-coded /tmp.
# The EXIT trap removes them on every exit path; INT and TERM become an exit.
TMP_ROOT="${TMPDIR:-$(dirname "$PROTOST")/Testing/Temporary}"
mkdir -p "$TMP_ROOT"
LOADFILE=""
SCRIPTFILE=""
trap 'rm -f "$LOADFILE" "$SCRIPTFILE"' EXIT
trap 'exit 130' INT
trap 'exit 143' TERM

run() { printf '%b' "$1" | "$PROTOST" -i 2>&1; }

# --- 1. banner is printed on start --------------------------------------------
out=$(run ':quit\n')
grep -q "interactive REPL" <<< "$out" || { echo "FAIL: banner missing"; exit 1; }

# --- 2. single expression evaluates immediately -------------------------------
out=$(run '3 + 4.\n:quit\n')
grep -q "=> 7" <<< "$out" || { echo "FAIL: 3+4 did not yield 7"; echo "$out"; exit 1; }

# --- 3. state persists across separate inputs ---------------------------------
out=$(run 'x := 21.\nx + x.\n:quit\n')
grep -q "=> 42" <<< "$out" || { echo "FAIL: state did not persist (x+x != 42)"; echo "$out"; exit 1; }

# --- 4. a user error does not kill the session --------------------------------
out=$(run 'garbage @@@.\n100 + 1.\n:quit\n')
grep -q "=> 101" <<< "$out" || { echo "FAIL: session died after an error"; echo "$out"; exit 1; }

# --- 5. multi-line class + method definition, then use it ---------------------
out=$(run "Object subclass: #ReplPt instanceVariableNames: ''.\nReplPt >> answer\n  ^ 99.\np := ReplPt newChild.\np answer.\n:quit\n")
grep -q "=> 99" <<< "$out" || { echo "FAIL: multi-line class+method def"; echo "$out"; exit 1; }

# --- 6. multi-line block (unclosed bracket forces continuation) ---------------
out=$(run 'r := [ :a :b |\n  a * b ] value: 6 value: 7.\n:quit\n')
grep -q "=> 42" <<< "$out" || { echo "FAIL: multi-line block"; echo "$out"; exit 1; }

# --- 7. unknown meta-command is reported, not evaluated -----------------------
out=$(run ':bogus\n2 + 2.\n:quit\n')
grep -q "unknown command: :bogus" <<< "$out" || { echo "FAIL: unknown command not reported"; echo "$out"; exit 1; }
grep -q "=> 4" <<< "$out" || { echo "FAIL: REPL did not continue after unknown command"; echo "$out"; exit 1; }

# --- 8. :help lists commands --------------------------------------------------
out=$(run ':help\n:quit\n')
grep -q ":quit" <<< "$out" || { echo "FAIL: :help did not list commands"; echo "$out"; exit 1; }

# --- 9. Ctrl-D (EOF) exits cleanly with code 0 --------------------------------
printf '5 + 5.\n' | "$PROTOST" -i >/dev/null 2>&1 || { echo "FAIL: EOF exit non-zero"; exit 1; }

# --- 10. :help lists every meta-command ---------------------------------------
out=$(run ':help\n:quit\n')
for c in ':load' ':reset' ':vars' ':time' ':help' ':quit'; do
    grep -q "$c" <<< "$out" || { echo "FAIL: :help missing $c"; echo "$out"; exit 1; }
done

# --- 11. :vars lists a user-defined variable ----------------------------------
out=$(run 'w := 123.\n:vars\n:quit\n')
grep -q "w = 123" <<< "$out" || { echo "FAIL: :vars did not list w"; echo "$out"; exit 1; }
# A pristine session has no user globals.
out=$(run ':vars\n:quit\n')
grep -q "no user-defined globals" <<< "$out" || { echo "FAIL: :vars on empty session"; echo "$out"; exit 1; }

# --- 12. :reset clears a previously-defined variable --------------------------
out=$(run 'k := 99.\n:reset\nk.\n:quit\n')
grep -q "session reset" <<< "$out" || { echo "FAIL: :reset not confirmed"; echo "$out"; exit 1; }
grep -qi "undefined global: k" <<< "$out" || { echo "FAIL: :reset did not clear k"; echo "$out"; exit 1; }
# The session is still usable after a reset.
out=$(run 'a := 1.\n:reset\n8 + 8.\n:quit\n')
grep -q "=> 16" <<< "$out" || { echo "FAIL: session unusable after :reset"; echo "$out"; exit 1; }

# --- 13. :time reports a time and the correct result --------------------------
out=$(run ':time 6 * 7\n:quit\n')
grep -q "=> 42" <<< "$out" || { echo "FAIL: :time wrong result"; echo "$out"; exit 1; }
grep -q "time:.*ms" <<< "$out" || { echo "FAIL: :time did not report a time"; echo "$out"; exit 1; }

# --- 14. :load executes a .st file into the current session -------------------
LOADFILE=$(mktemp "$TMP_ROOT/protost_replXXXXXX.st")
printf "loadedVar := 7 * 8.\n" > "$LOADFILE"
out=$(run ":load $LOADFILE\nloadedVar.\n:quit\n")
grep -q "loaded $LOADFILE" <<< "$out" || { echo "FAIL: :load not confirmed"; echo "$out"; exit 1; }
grep -q "=> 56" <<< "$out" || { echo "FAIL: :load definitions not available"; echo "$out"; exit 1; }
# A missing file is reported, not fatal.
out=$(run ':load /no/such/file.st\n2 + 2.\n:quit\n')
grep -q "cannot open" <<< "$out" || { echo "FAIL: :load missing file not reported"; echo "$out"; exit 1; }
grep -q "=> 4" <<< "$out" || { echo "FAIL: session died after bad :load"; echo "$out"; exit 1; }

# --- 15. regression: script execution and -e are unaffected -------------------
SCRIPTFILE=$(mktemp "$TMP_ROOT/protost_scriptXXXXXX.st")
printf "3 + 39.\n" > "$SCRIPTFILE"
out=$("$PROTOST" "$SCRIPTFILE") || { echo "FAIL: script execution exited non-zero"; exit 1; }
grep -qx 42 <<< "$out" || { echo "FAIL: script execution affected"; echo "$out"; exit 1; }
out=$("$PROTOST" -e "20 + 22.") || { echo "FAIL: -e exited non-zero"; exit 1; }
grep -qx 42 <<< "$out" || { echo "FAIL: -e affected"; echo "$out"; exit 1; }
# Meta-commands must not be recognised by -e (it is a plain expression path).
"$PROTOST" -e ":vars" >/dev/null 2>&1 && { echo "FAIL: -e treated :vars as a command"; exit 1; }

# --- S10. a block at the prompt assigns a session global ----------------------
# A top-level assignment binds a global; a block evaluated at the prompt reads
# it as a global, and an assignment in the block must update that same global
# instead of declaring a block-local variable.
out=$(run 's := 0.\n#(1 2) do: [ :x | s := s + x ].\ns.\n:quit\n')
grep -q "=> 3" <<< "$out" || { echo "FAIL: block did not update global s"; echo "$out"; exit 1; }
out=$(run 'g := 0.\n[ g := 7 ] value.\ng.\n:quit\n')
grep -q "=> 7" <<< "$out" || { echo "FAIL: block assignment to global g lost"; echo "$out"; exit 1; }
out=$(run 't := 1.\n[ [ t := t * 10 ] value ] value.\nt.\n:quit\n')
grep -q "=> 10" <<< "$out" || { echo "FAIL: nested block did not update global t"; echo "$out"; exit 1; }
# A block temporary of the same name still shadows the global.
out=$(run 'u := 5.\n[ | u | u := 99 ] value.\nu.\n:quit\n')
grep -q "=> 5" <<< "$out" || { echo "FAIL: block temporary did not shadow global u"; echo "$out"; exit 1; }
echo OK
