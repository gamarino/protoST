...
grep -q '"command":"initialize"' <<< "$out" \
    || { echo "FAIL: no initialize response"; echo "$out"; exit 1; }
grep -q '"type":"response"' <<< "$out" \
...
grep -q '"command":"someFutureCommand"' <<< "$out2" \
